...
tau = 7.0                       # nondim
L = 60.0                        # micrometers
nexact, kexact = 64, 128        # truncation

cases = [       # R, H, T, C1, C2, C3, C4, C5, C6
...
def eff_factor_fast(R, H, T, C):
    B = makematrix(T)
    EVs = factor(B)
    ufree = backsub(EVs, np.array(C)).reshape((-1,))
    ubar = ufree * np.array([
        1.0 if a2 == 0.0 else
        s(np.sqrt(a2), R, H, nexact, kexact, zero_cache=zc)
        for a2 in np.diag(B)
    ])
    etas = (
